...
class TrainingPipeline:
    ...
    def _train_models(self, sampling_strategies: Dict, X_test, y_test) -> Dict:
        ...
        for strategy_name, (X_sample, y_sample) in sampling_strategies.items():
            ...
            # Random Forest
            rf_model = trainer.get_random_forest(
                **self.config.get('models.random_forest', {})
            )
            rf_trained, rf_metrics = trainer.train_and_evaluate(
                rf_model, X_sample, y_sample, X_test, y_test,
                f'RF_{strategy_name}'
            )
            trained_models[f'RF_{strategy_name}'] = {
                'model': rf_trained,
                'metrics': rf_metrics
            }
            ...
            xgb_model = trainer.get_xgboost(
                ...
                **self.config.get('models.xgboost', {})
            )
            xgb_trained, xgb_metrics = trainer.train_and_evaluate(
                xgb_model, X_sample, y_sample, X_test, y_test,
                f'XGB_{strategy_name}'
            )
            trained_models[f'XGB_{strategy_name}'] = {
                'model': xgb_trained,
                'metrics': xgb_metrics
            }
            self.metrics_tracker.log_metrics(
    ...
    def _evaluate_and_select_best(self, trained_models: Dict, X_test, y_test) -> Dict:
        ...
        results_df = pd.DataFrame([
            info['metrics'] for info in trained_models.values()
        ])
    ...
    def _save_artifacts(self, best_model_info: Dict) -> None:
        ...
        save_model(
            ...
            models_dir / 'best_fraud_model.pkl'
        )
