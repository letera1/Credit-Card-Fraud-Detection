...
export default function RiskScoreCard({ score, maxScore, label }: RiskScoreCardProps) {
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 h-full flex flex-col items-center justify-center relative overflow-hidden group">
      ...
      <p className="text-2xs font-mono font-semibold text-muted-foreground uppercase tracking-widest absolute top-6 left-6 flex items-center gap-2">
      ...
      <div className="relative w-40 h-40 mt-6 flex items-center justify-center">
        <svg className="w-full h-full transform -rotate-90" viewBox="0 0 140 140">
          <circle cx="70" cy="70" r={radius} className="stroke-border/30" strokeWidth="12" fill="transparent" />
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-2xs text-muted-foreground font-mono mt-1">/100</span>
        </div>
      </div>

      <div className="mt-6 text-center z-10">
        <span className={`inline-flex items-center px-3 py-1 rounded-full text-2xs font-mono font-bold tracking-widest border border-current ${colorClass} bg-current/10`}>
          {labelText}
        </span>
      </div>
    </div>
