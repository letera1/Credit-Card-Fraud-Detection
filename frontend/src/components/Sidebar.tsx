...
export default function Sidebar({ activeView, setActiveView, collapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  const systemItems = [
    { id: 'overview', label: 'Overview', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z', badge: 0 },
    { id: 'analyze', label: 'Analyze Stream', icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z', badge: 0 },
    { id: 'batch', label: 'Batch Processing', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z', badge: 0 },
    { id: 'history', label: 'Transaction Logs', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', badge: 0 },
  ]

  const intelItems = [
    { id: 'analytics', label: 'Deep Analytics', icon: 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z', badge: 0 },
    { id: 'performance', label: 'Model Performance', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', badge: 0 },
    { id: 'alerts', label: 'Fraud Alerts', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', badge: 3 },
  ]

  const settingItems = [
    { id: 'model-info', label: 'Model Artifacts', icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z', badge: 0 },
  ...
  const getAllItems = () => [...systemItems, ...intelItems, ...settingItems]

  const NavItem = ({ item, isSetting = false }: { item: typeof systemItems[0]; isSetting?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <div ref={navRef} className="flex-1 overflow-y-auto scrollbar-hide py-4 px-2 space-y-6">
        {/* System */}
        <div>
        ...
        <div>
          ...
          <nav className="space-y-0.5">
            {intelItems.map((item) => <NavItem key={item.id} item={item} />)}
          </nav>
        </div>
        ...
        <div>
          ...
          <nav className="space-y-0.5">
            {settingItems.map((item) => <NavItem key={item.id} item={item} isSetting />)}
          </nav>
        </div>
      </div>
      ...
      <div className={`border-t border-border/40 transition-all duration-300 ${collapsed ? 'p-2' : 'p-3'}`}>
        ...
        >
          ...
          {!collapsed && (
            <span className="flex-1 text-left text-[13px]">{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</span>
          )}
        </button>
        ...
        {onToggleCollapse && (
          ...
          >
            ...
            {!collapsed && (
              <span className="flex-1 text-left text-[13px]">Collapse</span>
            )}
          </button>
        )}
