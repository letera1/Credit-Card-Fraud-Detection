...
export default function Header({ activeView, onOpenCommandPalette, onToggleMobileMenu, mobileMenuOpen }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-card/60 backdrop-blur-xl border-b border-border/30 h-14 flex items-center px-4 md:px-6 justify-between shrink-0">
      {/* Left: Mobile hamburger + Breadcrumb + View Title */}
      ...
      <div className="flex items-center gap-1 md:gap-2">
        ...
        <div className="hidden sm:flex items-center gap-1.5 px-2 py-1.5 rounded-lg hover:bg-muted/40 transition-colors cursor-default">
          <div className="flex h-1.5 w-1.5 relative">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${apiStatus === 'healthy' ? 'bg-green-400' : apiStatus === 'degraded' ? 'bg-yellow-400' : 'bg-red-400'}`} />
            <span className={`relative inline-flex rounded-full h-1.5 w-1.5 shadow-[0_0_6px_currentColor] ${apiStatus === 'healthy' ? 'bg-green-500 text-green-500' : apiStatus === 'degraded' ? 'bg-yellow-500 text-yellow-500' : 'bg-red-500 text-red-500'}`} />
          </div>
        ...
        <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-muted/50 transition-colors group" title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>
          {theme === 'dark' ? (
        ...
        <div className="relative" ref={notifRef}>
          <button onClick={() => { setShowNotifications(!showNotifications); setShowUserMenu(false) }} className="relative p-2 rounded-lg hover:bg-muted/50 transition-colors group">
            ...
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 min-w-[16px] h-4 flex items-center justify-center px-1 bg-red-500 rounded-full text-[9px] font-bold text-white font-mono border border-card shadow-[0_0_8px_rgba(239,68,68,0.6)]">{unreadCount}</span>
            )}
          </button>
            ...
            <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-card/95 backdrop-blur-2xl border border-border/50 rounded-xl shadow-2xl z-50 overflow-hidden animate-slide-down">
              <div className="px-4 py-3 border-b border-border/40 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {unreadCount > 0 && <span className="text-[10px] font-mono bg-red-500/10 px-1.5 py-0.5 rounded text-red-400 border border-red-500/20">{unreadCount} new</span>}
                </div>
                {unreadCount > 0 && <button onClick={markAllRead} className="text-[11px] font-medium text-purple-400 hover:text-purple-300 transition-colors">Mark all read</button>}
              </div>
              <div className="max-h-[360px] overflow-y-auto scrollbar-hide">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    <div key={notif.id} className={`group px-4 py-3 border-b border-border/30 hover:bg-muted/40 transition-colors cursor-pointer relative ${!notif.read ? 'bg-purple-500/[0.03]' : ''}`}>
                      {!notif.read && <div className="absolute left-1.5 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-purple-500" />}
                      <div className="flex items-start gap-3 pl-2">
                        <div className={`mt-1 flex-shrink-0 w-2 h-2 rounded-full shadow-[0_0_6px_currentColor] ${notif.type === 'critical' ? 'bg-red-500 text-red-500' : notif.type === 'warning' ? 'bg-orange-500 text-orange-500' : 'bg-blue-500 text-blue-500'}`} />
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className={`text-[9px] font-mono px-1 py-0.5 rounded ${notif.type === 'critical' ? 'bg-red-500/10 text-red-400' : notif.type === 'warning' ? 'bg-orange-500/10 text-orange-400' : 'bg-blue-500/10 text-blue-400'}`}>{notif.type}</span>
                          </div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); dismissNotification(notif.id) }} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-muted/60 transition-all flex-shrink-0">
                          <svg className="w-3 h-3 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                      </div>
                    </div>
                  ))
              ...
              <div className="px-4 py-2.5 border-t border-border/30">
                <button className="w-full text-center text-[11px] font-medium text-purple-400 hover:text-purple-300 transition-colors py-1">View all alerts</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={userRef}>
          <button onClick={() => { setShowUserMenu(!showUserMenu); setShowNotifications(false) }} className="flex items-center gap-2 p-1 rounded-lg hover:bg-muted/50 transition-colors group">
            ...
            <svg className="w-3 h-3 text-muted-foreground/50 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
          </button>
            ...
            <div className="absolute right-0 top-full mt-2 w-56 bg-card/95 backdrop-blur-2xl border border-border/50 rounded-xl shadow-2xl z-50 overflow-hidden animate-slide-down">
              <div className="px-4 py-3 border-b border-border/40">
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  ...
                  <span className="text-[10px] text-muted-foreground/30 mx-1">&middot;</span>
              ...
              <div className="py-1">
                {[
                  ...
                  <button key={item.label} className="w-full flex items-center gap-3 px-4 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors">
                    ...
                    {item.shortcut && <kbd className="text-[10px] font-mono text-muted-foreground/30 border border-border/40 rounded px-1 py-0.5">{item.shortcut}</kbd>}
                  </button>
                ))}
              </div>
